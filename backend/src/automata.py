...
class Automaton:
    ...
    def match(self, src: str, pos: int) -> Optional[Tuple[str, str, int]]:
        ...
        if is_letter(ch):
            ...
            if lexeme_raw in self.keywords:
                token_type = self.keywords[lexeme_raw]
                return (token_type, lexeme_raw, i - pos)
        ...
        if is_digit(ch):
            ...
            if i < len(src) and src[i] == '.':
                if (i + 1) < len(src) and is_digit(src[i + 1]):
                    i += 1  
                    while i < len(src) and is_digit(src[i]):
                        i += 1
                    lexeme = src[pos:i]
                    return ("NOMBRE_REEL", lexeme, i - pos)
                else:
                    lexeme = src[pos:i]
                    return ("NOMBRE_ENTIER", lexeme, i - pos)
            else:
                lexeme = src[pos:i]
                return ("NOMBRE_ENTIER", lexeme, i - pos)
